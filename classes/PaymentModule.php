			...
			if ($result AND isset($order->id))
			{
				...
				{					
					...
					else
					{
						...
						$reduction_amount = Tools::convertPrice($reduction_amount, $currency);
					}
					...
					$ecotaxRate = ($product['ecotax'] AND $ecotax = new Tax((int)Configuration::get('PS_ECOTAX_TAX_ID'))) ? Tax::getApplicableTax((int)$ecotax->id, (float)$ecotax->rate, (int)($order->id_address_delivery)) : 0.00;
					...
					$query .= '('.intval($order->id).',
						...
						'.floatval($tax).',
						'.Tools::convertPrice(floatval($product['ecotax']), intval($order->id_currency)).',
						'.floatval($ecotaxRate).',
					...
					if (isset($customizedDatas[$product['id_product']][$product['id_product_attribute']]))
					{
						...
						'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
							...
							<td style="padding: 0.6em 0.4em;"><strong>'.$product['name'].(isset($product['attributes_small']) ? ' '.$product['attributes_small'] : '').' - '.Tools::displayError('Customized').(!empty($customizationText) ? '<br />'.$customizationText : '').'</strong></td>
				...
				foreach ($discounts AS $discount)
				{
					...
					'<tr style="background-color:#EBECEE;">
							<td colspan="4" style="padding: 0.6em 0.4em; text-align: right;">'.Tools::displayError('Voucher code:').' '.$objDiscount->name.'</td>
				...
				if ($id_order_state != _PS_OS_ERROR_ AND $id_order_state != _PS_OS_CANCELED_ AND $customer->id)
				{
					...
					'{carrier}' => (!empty($carrier->name) ? $carrier->name : Configuration::get('PS_SHOP_NAME')),
