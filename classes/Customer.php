...
class		Customer extends ObjectModel
{
	...
	public function getStats()
	{
		...
		$id_guest = Db::getInstance()->getValue('SELECT id_guest FROM `'._DB_PREFIX_.'guest`WHERE `id_customer` = '.intval($this->id));
		$result['last_visit'] = Db::getInstance()->getValue('SELECT MAX(`date_add`) FROM `'._DB_PREFIX_.'connections` WHERE id_guest = '.intval($id_guest));

		$result['age'] = Db::getInstance()->getValue('
		SELECT (YEAR(CURRENT_DATE) - YEAR(`birthday`)) - (RIGHT(CURRENT_DATE, 5) < RIGHT(`birthday`, 5)) AS age
		FROM `'._DB_PREFIX_.'customer` WHERE `id_customer` = '.intval($this->id));
		if ($result['age'] == date('Y'))
			$result['age'] = '--';
	...
	*/	
	...
	public function addGroups($groups, $clean = false)
	{
		if ($clean)
			self::cleanGroups();
