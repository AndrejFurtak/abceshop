...
class		Product extends ObjectModel
{
	...
	*/
	public static function getPriceStatic($id_product, $usetax = true, $id_product_attribute = NULL, $decimals = 6, $divisor = NULL, $only_reduc = false, $usereduc = true, $quantity = 1, $forceAssociatedTax = false, $id_customer = NULL, $id_cart = NULL, $id_address_delivery = NULL)
	{
		...
		// Getting price
		$result = Db::getInstance()->getRow('
		SELECT p.`price`, p.`reduction_price`, p.`reduction_percent`, p.`reduction_from`, p.`reduction_to`, p.`ecotax`, p.`id_tax`, t.`rate`,
		...
		$attribute_price = ($usetax OR !Configuration::get('PS_TAX')) ? Tools::ps_round($attribute_price, 2) : ($attribute_price / (1 + (($tax ? $tax : $result['rate']) / 100)));
	...
	*/
	...
	public static function reinjectQuantities(&$orderDetail, $quantity)
	{
		...
		if (Pack::isPack(intval($orderDetail->product_id)))
		{
			...
			{
				Db::getInstance()->Execute('
				UPDATE `'._DB_PREFIX_.'product'.($orderDetail->id_product_attribute ? '_attribute' : '').'`
				SET `quantity` = `quantity`+'.intval($product_pack->pack_quantity * $quantity).'
				WHERE `id_product` = '.intval($product_pack->id).
				($orderDetail->id_product_attribute ? ' AND `id_product_attribute` = '.intval($product_pack->id_product_attribute) : ''));
				if ($orderDetail->id_product_attribute)
					Db::getInstance()->Execute('
					UPDATE `'._DB_PREFIX_.'product`
					SET `quantity` =
						(
