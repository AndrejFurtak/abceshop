...
class Loyalty extends Module
{
	...
	public function hookExtraRight($params)
	{
		...
		if (Validate::isLoadedObject($product))
		{
			...
			$smarty->assign(array('points' => intval($points), 'total_points' => intval($pointsAfter), 'voucher' => LoyaltyModule::getVoucherValue(intval($pointsAfter))));
	...
	/* Hook when admin validate return products */
	public function hookCancelProduct($params)
	{
		...
		WHERE `id_order` = '.intval($params['order']->id));
			
		$cancelQuantity = Tools::getValue('cancelQuantity');
		$cancelQuantity = $cancelQuantity[$params['id_order_detail']];
		
		
		// Case all product of an order were returned
		$result = Db::getInstance()->getRow('
		SELECT IF(SUM(`product_quantity_return`) = SUM(`product_quantity`),0,1) AS remaining 
		FROM `'._DB_PREFIX_.'order_detail`
		WHERE `id_order` = '.intval($params['order']->id));
		if (intval($result['remaining']) == 0)
		{
			$loyalty->points = 0;
			$loyalty->id_loyalty_state = LoyaltyStateModule::getCancelId();
		}
		...
		{
			$result = Db::getInstance()->getRow('
			SELECT `product_price` * (1 + (`tax_rate` / 100)) * (1 - (`reduction_percent` / 100))	AS total 
			FROM `'._DB_PREFIX_.'order_detail`
			WHERE `id_order_detail` = '.intval($params['id_order_detail']));
			
			// directly decrements the points
			$pointsToSubstract = LoyaltyModule::getNbPointsByPrice($result['total'] * intval($cancelQuantity));
			$loyalty->points -= $pointsToSubstract;
		}
		$loyalty->save();
		return;
	}
