...
class LoyaltyModule extends ObjectModel
{
	...
	static public function getOrderNbPoints($order)
	{
		...
		SELECT (product_price * (1 + (tax_rate / 100)) * (1 - (reduction_percent / 100))) AS total, product_quantity AS quantity
		FROM '._DB_PREFIX_.'order_detail
		WHERE id_order = '.intval($order->id));
		
		foreach ($return AS $line)
			for ($i = 0 ; $i < $line['quantity'] ; $i++)
				$total += self::getNbPointsByPrice($line['total']);
		return $total;
	}
	...
	static public function getCartNbPoints($cart, $newProduct = NULL)
	{
		...
		if (Validate::isLoadedObject($cart))
		{
			...
			{
				$cartProductsNew['id_product'] = intval($newProduct->id);
	...
	static public function getVoucherValue($nbPoints)
	{
		...
		return floatval(floatval($nbPoints) * floatval(Tools::convertPrice(Configuration::get('PS_LOYALTY_POINT_VALUE'), $currency)));
	}
	...
	static public function getNbPointsByPrice($price)
	{
		...
		if ($pointRate = floatval(Configuration::get('PS_LOYALTY_POINT_RATE')))
			$points = Tools::ps_round($price / $pointRate, 0);
		return $points;
	}
